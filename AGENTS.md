...
每篇 `notes/weekN/dayN.md` 必须包含下面四块内容，缺少任何一块都视为这一天的笔记还没有完成；其中前三块属于「概念」部分，第四块附在笔记末尾：
...
| 4 | 十分钟速记卡（附在笔记末尾） | 面向复习，允许与前面的小节重复。必须包含：每个对象的一句话定义、必须记住的字段、全流程链路摘要、当天实测出来的硬结论（写明具体取值）、面试问答卡的标准答法、出现异常时的排查顺序、最容易答错的若干点、合上笔记自测的问题。 |
